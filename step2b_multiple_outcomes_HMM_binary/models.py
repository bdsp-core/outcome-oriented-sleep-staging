...
class MyDataset(Dataset):
    ...
    def collate(cls, batch, split_to_shorter=False):
        ...
        if has_y:
            return (th.tensor(x_pr).float(), th.tensor(x_pl).float(), th.tensor(T).long(), th.tensor(batch_y))
        else:
            return (th.tensor(x_pr).float(), th.tensor(x_pl).float(), th.tensor(T))
...
class HMMOOSSClassifier(BaseEstimator, ClassifierMixin, LightningModule):
    ...
    def fit(self, X, Y, separate=False):
        ...
        if os.path.exists(path):
            ...
            self.coef_th_ = nn.Parameter(th.randn(NC-1,self.Dy)*scale)
    ...
    def score(self, X=None, y=None, return_components=False):
        ...
        if return_components:
            return np.mean(auc), auc#+emission_prob_divergence
        else:
            return np.mean(auc)
