...
def main():
    ...
    random_state = 2023
    result_folder = f'results_aasm_multiple_outcomes_epoch{epoch_time}s'
    ...
    Y_all = df_y[ycols].values#.astype(int)
    N = len(Y_all)
    print(f'N = {N}')
    ...
    for yi, ycol in enumerate(ycols):
        ...
        Y = Y_all[:,yi]
        df_cv = df_cv_all.copy()
        df_cv.loc[np.isnan(Y), 'CV'] = np.nan
        
        yptes_aasm = np.zeros(N)
        models_aasm_cv = []
        for cvi in range(1,Ncv+1):
            print(f'AASM model CV = {cvi}')
            trids = np.where(np.in1d(df_cv.CV, [x for x in range(1,Ncv+1) if x!=cvi]))[0]
            teids = np.where(df_cv.CV==cvi)[0]
            Str = [S[i] for i in trids]
            ytr = Y[trids].astype(int)
        ...
        ids = (~np.isnan(Y))&(~np.isnan(yptes_aasm))
        auc = roc_auc_score(Y[ids], yptes_aasm[ids])
        print(f'{ycol}: OVERALL CV AUC AASM = {auc}')
        ...
        X = np.array([get_features(s) for ii, s in enumerate(S) if ii in ids])
        model_aasm_final.fit(X, Y[ids])
        print(f'Final AASM coef = {model_aasm_final.coef_}')
        
        os.makedirs(result_folder, exist_ok=True)
        with open(os.path.join(result_folder, f'results_aasm_{ycol}.pickle'), 'wb') as ff:
            pickle.dump({
