...
def main():
    ...
    Ncv = 10
    #W = 6000
    W = [100]*6 + [1]*3 + [1]*3 + [1]*1 + [10]*1
    n_jobs = 8
    
    ncs = [10]#np.arange(3,11)
    #teids_cv = []
    sids_te = []
    ytes = []
    yptes_new = []
    ...
    for cvi, (trids, teids) in enumerate(cvf.split(np.zeros((len(X),1)), Y)):
        ...
        for nc in ncs:
            ...
            model_Y = BayesSearchCV(model_Y, {
                ...
                scoring='roc_auc', cv=Ncv, n_points=n_jobs, n_iter=50,
    ...
    Z = [Z[ids[i]:ids[i+1]] for i in range(len(ids)-1)]
    Zp = model_Z_final.predict_proba(X2, lengths=T)
    Zp = [Zp[ids[i]:ids[i+1]] for i in range(len(ids)-1)]
